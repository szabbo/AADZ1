...
fun main() {
    ...
    solveChallengeFour(data)
}

fun solveFirst(someData: Map<String, List<Int>>) = println("Average age of users is: ${someData.flatMap { it.value }.filter { it > 0 }.map { it }.average()}")

fun solveSecond(someData: Map<String, List<Int>>){
    for (someItem in someData){
        someItem.value.forEach {
            when{
                it < 0 -> println("Huston we have a problem with ${someItem.key}!!!")
            }
        }
    }
}

fun solveThird(someData: Map<String, List<Int>>){
    var numberOfFaultyItems = 0
    for (someItem in someData.flatMap { it.value }){
        when{
            someItem < 0 -> numberOfFaultyItems++
        }
    }

    println("Number of faulty items is: $numberOfFaultyItems")
}

fun solveChallengeFour(someData: Map<String, List<Int>>){
    solveFirst(someData)
    solveSecond(someData)
    solveThird(someData)
}